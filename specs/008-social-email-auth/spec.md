...
**Created**: 2026-04-08  
**Status**: Draft  
**Input**: User description: "preciso além de aceitar login pelo Telegram também aceitar via providers (Social Sign-On) com Google e via magic link por email"

## Clarifications

### Session 2026-04-08

- Q: Como funciona o account linking entre Telegram e outros providers? → A: Telegram (telefone) e providers baseados em email (Google, Magic Link) são espaços de identidade separados. Auto-link só funciona entre providers que compartilham email verificado (Google ↔ Magic Link). Telegram + provider de email = contas distintas, a menos que o provider forneça telefone que dê match com o número do Telegram.
- Q: Como deve ser a hierarquia visual dos métodos de login? → A: Social primeiro — botão Google no topo (mais rápido, 1 clique), separador "ou", depois email magic link, e por último Telegram/telefone como opção secundária.
...
### User Story 2 - Email Magic Link (Priority: P2)

A user who doesn't have Telegram or Google can sign in using their email address. On the login page, the user enters their email, receives a magic link via email, clicks the link, and is signed in with an active session.
...
### User Story 3 - Unified Login Page (Priority: P1)

The login page presents all available authentication methods in a clear, organized layout. Users can choose between Telegram (existing), Google, or email magic link to sign in or create an account.
...
**Independent Test**: Can be tested by visiting the login page and verifying all three sign-in options are visible and clearly labeled.

**Acceptance Scenarios**:

1. **Given** an unauthenticated user, **When** they visit the login page, **Then** they see the Google button at the top, followed by a separator, then email magic link input, and finally Telegram (phone number) as a secondary option.
2. **Given** a user on a mobile device, **When** they view the login page, **Then** all sign-in options are accessible and properly laid out with the social provider most prominent.
...
A user who has signed in with one email-based method (Google or Magic Link) can also sign in with a different email-based method using the same email. The system automatically links accounts that share the same verified email address across these providers. Telegram (phone-based) accounts are a separate identity space and are not auto-linked to email-based accounts.

**Why this priority**: Prevents duplicate accounts among email-based providers and ensures users don't lose access to their data when switching between Google and email magic link.
...
2. **Given** a user with multiple linked email-based providers, **When** they sign in via any of their linked methods, **Then** they always access the same unified account.
3. **Given** a user who signed up via Telegram (phone), **When** they sign in via Google with a different email, **Then** a separate account is created — the two are not linked.

---

### Edge Cases

- What happens when a user tries to link a Google account whose email is already associated with a different user? → The system denies the link and shows an error message explaining the email is already in use by another account.
- What happens when a user has a Telegram account and also signs in via Google/Magic Link? → Two separate accounts are created. Telegram (phone-based) and email-based providers are separate identity spaces. Linking only happens automatically between email-based providers sharing the same verified email.
...
- What happens during Google OAuth if the provider is temporarily unavailable? → The user sees a friendly error message and is encouraged to try again or use a different sign-in method.
...
- **FR-002**: System MUST support email-based magic link authentication.
- **FR-003**: System MUST auto-create accounts for new users signing in via any provider.
- **FR-004**: System MUST automatically link accounts that share the same verified email address across email-based providers (Google, Magic Link). Telegram (phone-based) accounts exist as a separate identity space and are NOT auto-linked to email-based accounts.
- **FR-005**: System MUST preserve existing Telegram phone-number OTP authentication alongside new methods.
- **FR-006**: System MUST redirect new users (without a display name) to the complete-profile page after sign-in, regardless of the authentication method used.
- **FR-007**: System MUST populate user profile fields (name, email, avatar) from provider data when available.
- **FR-008**: System MUST stop generating real-looking fake emails for Telegram-only users. Telegram accounts should use an inert sentinel email on a non-routable domain (e.g., `@phone.noemail.internal`) that will never match real emails from social/magic-link providers. Combined with `emailVerified: false` and exclusion from `trustedProviders`, this prevents accidental account linking.
...
- **FR-011**: System MUST show clear error messages when OAuth flow fails or is cancelled.
- **FR-012**: System MUST rate-limit magic link requests to prevent abuse (max 3 requests per email per 5 minutes).
- **FR-013**: Login page MUST display all available sign-in options in a clear, accessible layout.
- **FR-014**: System MUST maintain the same session behavior (90-day expiry, 24-hour refresh) for all authentication methods.
...
- **Account**: Existing entity; stores provider-specific credentials (Google, Telegram). A user can have multiple accounts (one per provider).
...
- **SC-001**: Users can complete sign-in via Google in under 30 seconds (excluding provider consent screen time on first use).
...
- Google Cloud account is available and configured for OAuth credentials.
