...
function CreatePoolPage() {
  ...
  async function handleCreate() {
    ...
    try {
      ...
      if (!res.ok) { const data = await res.json(); setError(data.message || 'Erro ao criar bolao'); return }
    ...
    return (
      <div className="flex flex-col gap-6">
        <div>
          ...
          <h1 className="mt-1 font-display text-4xl font-black leading-[0.9] text-black">Bolao Criado</h1>
  ...
  return (
    <div className="flex flex-col gap-8">
      <div>
        ...
        <h1 className="mt-1 font-display text-4xl font-black leading-[0.9] text-black">Criar Bolao</h1>
        <div className="mt-3 h-1 w-12 bg-red" />
      </div>

      <Input label="Nome do bolao" placeholder="Ex: Bolao da Galera" value={name} onChange={(e) => setName(e.target.value)} maxLength={50} />
