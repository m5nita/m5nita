...
function PredictionsPage() {
  ...
  const { data: matchesData, isPending: matchesPending } = useQuery({
    ...
    queryFn: async () => {
      const res = await fetch('/api/matches?stage=group', { credentials: 'include' })
  ...
  const predictionMap = new Map(
    predictions.map((p: any) => [p.matchId, p])
  )

  const groupMatches = matches.filter((m: any) => m.group === activeGroup)
